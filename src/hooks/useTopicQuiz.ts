...
export type TopicQuizUserAnswer = {
  questionId: number;
  selected: AnswerLetter;
  isCorrect: boolean;
};

export function useTopicQuiz(topicId: TopicId) {
  const { user, isGuest } = useAuth();
  const queryClient = useQueryClient();

  const questionsQuery = useTopicQuestions(topicId, QUIZ_TOTAL);
  ...
  const [userAnswers, setUserAnswers] = useState<TopicQuizUserAnswer[]>([]);
  ...
  const streakMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: queryKeys.userAnswers.byUserTopicStats(user.id),
      });
    },
    onError: (err) => {
